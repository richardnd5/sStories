...
class CatchingMelodies: UIView, CatchingMelodyProtocol {
    ...
    func createInstructionLabel() {
        ...
        instructionLabel?.anchor(
            ...
            bottom: nil,
            padding: UIEdgeInsets(top: y, left: sidePadding, bottom: 0, right: -sidePadding),
            size: .zero
        )
    ...
    func missingMelodyType(melodyArray: [MelodyImage]) -> MelodyType {
        ...
        let containsBegin = melodyArray.contains { $0.type == .begin }
        let containsMiddle = melodyArray.contains { $0.type == .middle }
        let containsTonic = melodyArray.contains { $0.type == .tonic }
        let containsDominant = melodyArray.contains { $0.type == .dominant }
        let containsEnding = melodyArray.contains { $0.type == .ending }
        let containsFinal = melodyArray.contains { $0.type == .final }
    ...
    func removeImagesFromCaughtMelodyScene() {
        ...
        sack?.fadeAndRemove(time: 1.0)
        throwbackWater?.fadeAndRemove(time: 1.0)
        keepLabel?.fadeAndRemove(time: 1.0)
        throwbackLabel?.fadeAndRemove(time: 1.0)
    }
